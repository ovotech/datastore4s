...
import com.ovoenergy.datastore4s.internal.{
  DatastoreError,
  ProjectionEntity,
  ValueFormat,
  WrappedEntity
...
trait Query[E] {
  ...
  def withPropertyEq[A](propertyName: String, value: A)(
      implicit valueFormat: ValueFormat[A])
    : Query[E] // TODO extend this somehow. Perhaps with a DSL?
...
object Query {
  def ancestorToKey(ancestor: Ancestor,
                    keyFactory: com.google.cloud.datastore.KeyFactory): Key =
    ancestor match {
...
case class DatastoreQuery[E](
    queryBuilder: StructuredQuery.Builder[_ <: BaseEntity[Key]],
    entityFunction: EntityFunction = WrappedEntity(_))(
  ...
  override def withPropertyEq[A](propertyName: String, value: A)(
      implicit valueFormat: ValueFormat[A]) =
    DatastoreQuery(
      queryBuilder.setFilter(
        PropertyFilter.eq(propertyName, valueFormat.toValue(value).dsValue)),
      entityFunction)
...
case class Project[E]()(implicit datastore: Datastore,
                        format: EntityFormat[E, _]) {
  def into[A]()(implicit fromEntity: FromEntity[A]) = Projection[E, A]()
}

case class Projection[E, A]()(implicit datastore: Datastore,
                              format: EntityFormat[E, _],
                              fromEntity: FromEntity[A]) {
  def mapping(firstMapping: (String, String),
              remainingMappings: (String, String)*): Query[A] = {
    ...
    DatastoreQuery[A](
      queryBuilder,
      (e: BaseEntity[Key]) => ProjectionEntity(mappings, WrappedEntity(e)))
  }
}
