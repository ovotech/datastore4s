...
object DatastoreError {
  def missingField[A](fieldName: String,
                      entity: Entity): Either[DatastoreError, A] =
    Left(new DatastoreError {
      override def toString: String =
        s"Field $fieldName could not be found on entity $entity"
    })

  def wrongType[A](expectedType: DsType,
                   datastoreValue: DatastoreValue): Either[DatastoreError, A] =
    Left(new DatastoreError {
      override def toString: String =
  ...
  def sequence[A](values: Seq[Either[DatastoreError, A]])
    : Either[DatastoreError, Seq[A]] = {
