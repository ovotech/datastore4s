...
object ValueFormat {
  ...
  implicit def entityValueFormat[E, K](implicit toEntityComponents: ToEntityComponents[E, K],
                                       fromEntity: FromEntity[E],
                                       datastoreService: DatastoreService,
                                       toKey: ToKey[K]): ValueFormat[E] =
    new ValueFormat[E] {
      override def toValue(scalaValue: E): DatastoreValue =
        EntityValue(DatastoreService.toEntity(scalaValue, toEntityComponents, datastoreService))

      override def fromValue(datastoreValue: DatastoreValue): Either[DatastoreError, E] = datastoreValue match {
        case EntityValue(entity) => fromEntity.fromEntity(entity)
